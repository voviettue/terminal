...
describe('getPackageExtensions', () => {
	...
	it('returns an array of extensions based on package.json', async () => {
		...
		ensureDirSync(`${childPackage.name}/@giga-extensions/test/`);
		writeJsonSync(`${childPackage.name}/@giga-extensions/test/package.json`, {
			name: 'test',
			version: '0.1',
			dependencies: {},
			'directus:extension': { type: 'pack', path: './', source: 'test', host: '^9.0.0' },
		...
		expect(await getPackageExtensions(childPackage.name, EXTENSION_PACKAGE_TYPES)).toStrictEqual([
			...
			},
			{
				...
				host: '^9.0.0',
				local: false,
				name: '@giga-extensions/test',
				path: childPackage.name + '/@giga-extensions/test',
				type: 'pack',
				version: '0.1',
			},
		]);
